...
def start(message):
    ...
    bot.send_photo(message.chat.id, open('../bot-kvest/рюкзак.png', 'rb'), caption=f"Привет, {message.from_user.first_name}! Готов погрузиться в мир путешествий с разными героями? Если да, то погналии...\nЗдесь ты будешь выбирать героев и их предметы, а в зависимости от выбора местности твое путешествие может закончиться нахождением сокровища или не совсем хорошо....", reply_markup=markup)
...
def start(message):
    ...
    bot.send_photo(message.chat.id, open('../bot-kvest/рюкзак.png', 'rb'), caption=f"{message.from_user.first_name} захотел пройти квест еще раз? Окей)", reply_markup=markup)
    item[message.chat.id] = {"item": ""}
    user_id = message.from_user.id
    players[user_id] = user_id
    players[user_id] = {'level': 1, 'name': message.from_user.first_name}
...
def description(message):
    ...
    if message.text == 'Принцесса Эмеральда':
        ...
        bot.send_photo(message.chat.id, open('../bot-kvest/Emeralda.png', 'rb'), caption=plan_action["2"])
        ...
        bot.send_photo(message.chat.id, open('../../kvest_bot/bot-kvest/blacktorn.png', 'rb'), caption=plan_action["3"])
        ...
        bot.send_photo(message.chat.id, open('../bot-kvest/jon.png', 'rb'), caption=plan_action["4"])
        ...
        bot.send_photo(message.chat.id, open('../bot-kvest/люди.png', 'rb'), caption=plan_action["1"], reply_markup=a)
    elif message.text == "Пустыни Египта":
        remove_keyboard = types.ReplyKeyboardRemove()
        if item[message.chat.id]["item"] in items_dissert:
            bot.send_photo(message.chat.id, open('../bot-kvest/dissert.png', 'rb'), caption=plan_action["7"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Бутылка воды":
            bot.send_photo(message.chat.id, open('../bot-kvest/dissert.png', 'rb'), caption=plan_action["8"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Кактус":
            bot.send_photo(message.chat.id, open('../bot-kvest/dissert.png', 'rb'), caption=plan_action["9"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Фильтр":
            bot.send_photo(message.chat.id, open('../bot-kvest/dissert.png', 'rb'), caption=plan_action["10"], reply_markup=remove_keyboard)
        ...
        if item[message.chat.id]["item"] in items_swamp:
            bot.send_photo(message.chat.id, open('../bot-kvest/swamp.png', 'rb'), caption=plan_action["11"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Канат":
            bot.send_photo(message.chat.id, open('../bot-kvest/swamp.png', 'rb'), caption=plan_action["12"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Сапоги":
            bot.send_photo(message.chat.id, open('../bot-kvest/swamp.png', 'rb'), caption=plan_action["14"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Волшебная палочка":
            bot.send_photo(message.chat.id, open('../bot-kvest/swamp.png', 'rb'), caption=plan_action["13"], reply_markup=remove_keyboard)
        bot.send_message(message.chat.id, "Если хочешь пройти игру заново нажми /again")
        ...
        if item[message.chat.id]["item"] in items_cave:
            bot.send_photo(message.chat.id, open('../../kvest_bot/bot-kvest/cave.png', 'rb'), caption=plan_action["15"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Лук(оружие)":
            bot.send_photo(message.chat.id, open('../../kvest_bot/bot-kvest/cave.png', 'rb'), caption=plan_action["16"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Шлем":
            bot.send_photo(message.chat.id, open('../../kvest_bot/bot-kvest/cave.png', 'rb'), caption=plan_action["17"], reply_markup=remove_keyboard)
        elif item[message.chat.id]["item"] == "Меч":
            bot.send_photo(message.chat.id, open('../../kvest_bot/bot-kvest/cave.png', 'rb'), caption=plan_action["18"], reply_markup=remove_keyboard)
        bot.send_message(message.chat.id, "Если хочешь пройти игру заново нажми /again")
    else:
        ...
        bot.send_photo(message.chat.id, open('../bot-kvest/place.png', 'rb'), caption=plan_action["6"], reply_markup=travel)
